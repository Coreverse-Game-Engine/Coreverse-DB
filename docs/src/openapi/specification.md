...
- **`info`** — title, description, version (currently `0.4.0`), and license (`GPL-3.0-only`).
...
- **`tags`** — one per Edge Function/resource (`auth`, `releases`, `teams`, `requests`, `profiles`, `projects`, `news`, `polls`, `discussions`, `docs`), each with a one-line description.
