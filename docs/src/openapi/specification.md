...
- **`info`** — title, description, version (currently `0.2.0`), and license (`GPL-3.0-only`).
...
- **`tags`** — one per Edge Function/resource (`releases`, `teams`, `requests`, `profiles`, `projects`, `news`, `polls`, `discussions`, `docs`, `auth`), each with a one-line description.
