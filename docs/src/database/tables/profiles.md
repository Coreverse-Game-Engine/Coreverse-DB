...
| `full_name` | `text` | Defaults to `''`, populated from `auth.users.raw_user_meta_data->>'full_name'` at signup if present |
| `avatar_path` | `text`, nullable | Storage path in the `avatars` bucket; PNG-only, enforced at bucket policy + Edge Function level |
