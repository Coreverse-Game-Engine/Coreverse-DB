...
| `trg_handle_new_auth_user` | `auth.users` (`after insert`) | `identity.handle_new_auth_user()` | Auto-creates the matching `identity.profiles` row on signup, seeding `full_name` from `raw_user_meta_data` if present |
